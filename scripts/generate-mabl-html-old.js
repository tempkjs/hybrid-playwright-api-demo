...
${data.tests.map(t => `
  <li>
    <a href="./${t.testInfo.summary}.html">${t.testInfo.summary}</a>
...
data.tests.forEach(test => {
  ...
  <html>
  <head>
    <title>${test.testInfo.summary}</title>
  </head>
  <body>
    <h1>${test.testInfo.summary}</h1>
  ...
  fs.writeFileSync(`${REPORT_DIR}/${test.testInfo.summary}.html`, testHtml);
});
